...
{    
	...
	double fit = ( m_fComputedFitness / (double) m_unNumberOfSteps ) * ( (double) (fmin(contador, 8.0)/ 8.0 )) * ( (double) (fmin(contador_bateria, 8.0)/ 8.0 )) * (1 - ((double) (fmin(coll,10.0)/10.0)));
...
void CIriFitnessFunction::SimulationStep(unsigned int n_simulation_step, double f_time, double f_step_interval)
{
	...
	int flag2=0;
  	...
  	if ( blueBattery[0] < 0.7){
    		fitness = fitness + (coef3 * ( blueLightS0 + blueLightS7)) - (coef2 *(maxLightSensorEval)) - (coef1 * (0.7 -blueBattery[0]));
		flag2=1;
	}
 	 else {
	...
	//if(((maxBlueLightSensorEval)>0.9)||((maxLightSensorEval) > 0.9)){
	//contador ++;
	//}
